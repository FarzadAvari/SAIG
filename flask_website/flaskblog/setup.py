#!/usr/bin/python3
# -*- coding: utf-8 -*-
# Created by: python.exe -m py2exe run.py -W setup.py
...
run = Target(
    ...
    script="run.py", # path of the main script

    # Allows to specify the basename of the executable, if different from 'run'
    # dest_base = "run",

    # Icon resources:[(resource_id, path to .ico file), ...]
    # icon_resources=[(1, r"run.ico")]

    other_resources = [(RT_MANIFEST, 1, (manifest_template % dict(prog="run", level="asInvoker")).encode("utf-8")),
